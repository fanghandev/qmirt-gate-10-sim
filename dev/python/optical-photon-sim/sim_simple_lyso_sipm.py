...
def add_sim_geometry(
    ...
    optical_epoxy.size = [0.1 * mm, 3 * mm, 3 * mm]
    optical_epoxy.translation = [5.05 * mm, 0 * mm, 0 * mm]
    optical_epoxy.mother = sim.volume_manager.get_volume("world")

    # Add a box SiPM volume
    sipm = sim.volume_manager.add_volume("Box", "sipm")
    sipm.size = [1 * mm, 3 * mm, 3 * mm]
    sipm.translation = [5.6 * mm, 0 * mm, 0 * mm]
...
def add_sim_actors(sim: gate.Simulation):
    ...
    crystal_phsa.output_filename = "lyso_sipm_phsa_output.root"

    opt_phsa = sim.add_actor("PhaseSpaceActor", "opt_phsa")
    opt_phsa.attached_to = sim.volume_manager.get_volume("optical_epoxy")
    opt_phsa.attributes = [
        "EventID",
        "ParentID",
        "TrackID",
    ...
    opt_phsa.steps_to_store = "entering"
    opt_phsa.output_filename = "lyso_sipm_phsa_output.root"

    sipm_phsa = sim.add_actor("PhaseSpaceActor", "sipm_phsa")
    sipm_phsa.attached_to = sim.volume_manager.get_volume("sipm")
    sipm_phsa.attributes = [
        "EventID",
        "ParentID",
        "TrackID",
        "CurrentStepNumber",
    ...
    sipm_phsa.steps_to_store = "entering"
    sipm_phsa.output_filename = "lyso_sipm_phsa_output.root"
