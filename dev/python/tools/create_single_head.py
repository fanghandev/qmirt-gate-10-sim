...
if __name__ == "__main__":
    ...
    df_values = pd.read_excel(xlsx_path, sheet_name="Values")  # read the "Values" sheet
    df_values.columns = df_values.iloc[0]
    df_values = df_values[1:]  # remove the first row which is now the header
    ...
    df_values.columns.name = "Values Sheet"
    ...
    hole_fov_center_distance_mm_np = np.linalg.norm(collimator_hole_coords_mm, axis=1)
    azmuthal_angle_deg = (
        np.arctan2(collimator_hole_coords_mm[:, 1], collimator_hole_coords_mm[:, 0])
        * 180
        / np.pi
    )
    hole_fov_center_dist_xy_mm_np = np.linalg.norm(
        collimator_hole_coords_mm[:, :2], axis=1
    )
    polar_angle_deg = (
        np.arctan2(collimator_hole_coords_mm[:, 2], hole_fov_center_dist_xy_mm_np)
        * 180
        / np.pi
    )
    ...
    collimator_body_inner_top_mm_np = np.full((80,), detector_crystal_size_mm)
    ...
    sim.user_info.visu_filename = "../collimator_geometry.wrl"  # type: ignore
    sim.run()
