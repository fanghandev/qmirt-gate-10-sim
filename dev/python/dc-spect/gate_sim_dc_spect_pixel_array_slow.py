...
def add_dc_spect_geometry(sim: gate.Simulation, config: dict, persist_data_dir: Path):
    ...
    for i in range(80):
        ...
        detector_crystal.size = pixel_size_mm

        print(f"Detector crystal size for head {i + 1}: {detector_crystal.size} mm")
        ...
        detector_crystal.rotation = array_rotation
...
def run_simulation(config: dict, persist_data_dir: Path, args):
    ...
    detector_hits_actor: gate.actors.digitizers.DigitizerHitsCollectionActor = (
        sim.add_actor("DigitizerHitsCollectionActor", "DetectorHitsActor")
    )
