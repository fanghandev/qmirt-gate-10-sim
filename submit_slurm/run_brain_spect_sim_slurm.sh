...
DATA_DIR="results/brain_spect/slurm/${BATCH_ID}"
SBATCH_FILE="${LOG_DIR}/brain_spect_sim_slurm.sbatch"

mkdir -p "$LOG_DIR" "$DATA_DIR"

cat > "$SBATCH_FILE" <<EOF
#!/bin/bash
#SBATCH --job-name=brain-spect
...
"$SCRIPT_DIR/wrapper_brain_spect_sim_slurm.sh"
EOF
