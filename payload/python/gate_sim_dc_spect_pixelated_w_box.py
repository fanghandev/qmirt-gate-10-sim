...
def get_dc_spect_geometry_config(xlsx_path: Path, stl_dir: Path) -> dict:
    n_heads = 80
...
def construct_collimator_box_gate_geometry(config: dict, id: int):
    ...
    # Add a box with 1 mm thickness around th crystal.
    ...
    box_outer_size_mm = np.full(
        (3,), config["collimator_body_outer_top_mm_np"][id] + 4.0
    )
    # add 2 mm to ensure the box fully covers the crystal
    box_outer_size_mm[2] = config["detector_crystal_size_mm"][2] + 2.0
    box_inner_size_mm = np.full(
        (3,), config["collimator_body_outer_top_mm_np"][id] + 2.0
    )

    box_inner_size_mm[2] = config["detector_crystal_size_mm"][2] + 2.0
...
def add_pixlate_detector_to_gate_sim(sim: gate.Simulation, config: dict, id: int):
    r = get_head_rotation_matrix(config, id)
...
def add_sheilding_to_gate_sim(sim: gate.Simulation, config: dict):
...
def save_simulation_geometry_to_wrl(config: dict, persist_data_dir: Path, args):
    ...
    for i in range(80):
        ...
        add_pixlate_detector_to_gate_sim(sim, config, id=i)
    add_sheilding_to_gate_sim(sim, config)  # Add the shielding as an example
...
def render_wrl_to_html(wrl_path: Path, html_output_path: Path):
    ...
    wrl_path = "/home/fanghan/Work/RPIL/QMIRT/gate10mc/dev/collimator_geometry.wrl"
...
def save_simulation_geometry_to_wrl(config: dict, persist_data_dir: Path, args):
    ...
    print("Storing geometry into wrl file only without running the simulation...")
    sim.user_info.visu_filename = str(
        (persist_data_dir.parent / "dev" / "dc_spect_geometry.wrl").resolve()
    )
    print(f"Geometry stored in {sim.user_info.visu_filename}")
    sim.run()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=1,
        required=False,
        help="Number of threads requested.",
    )
    parser.add_argument(
        "-x",
        "--xlsx-path",
    ...
    config = get_dc_spect_geometry_config(
        xlsx_path, stl_dir=persistent_data_dir / "stl"
    )
