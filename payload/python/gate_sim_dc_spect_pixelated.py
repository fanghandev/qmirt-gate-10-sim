...
def add_crystal_box(sim: gate.Simulation, name: str):
    ...
    crystal_box.size = [52.0 * mm, 52.0 * mm, 12.0 * mm]  # unit is mm
...
def add_dc_spect_geometry(sim: gate.Simulation, config: dict, persist_data_dir: Path):
    ...
    #     size=n_pixels, spacing=pixel_size_mm
    # )
    for i in range(80):
        ...
        # rotated_global_array_translations = [
        #     Rotation.from_matrix(r).apply(local_translation)
        #     + detector_crystal_translation_mm[i]
        #     for local_translation in local_array_translations
        # ]

        # detector_crystal.translation = rotated_global_array_translations
        # array_rotation = [r] * n_pixels.prod()
        ...
        # repeat_param_vol.repeated_volume_name = f"pixel_{i + 1}"
...
def add_box_source(
    ...
    source.position.size = [170.1, 170.1, 170.1]  # unit is mms
