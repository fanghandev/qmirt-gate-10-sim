...
def add_pixelated_detector_to_gate_sim(sim: gate.Simulation, config: dict, id: int):
    ...
    # pixel_repeater.mother = "world"
    # print(f"Pixel size for head {i + 1}: {detector_pixel.size} mm")
...
app = typer.Typer(add_completion=True, help="Run the DC-SPECT simulation")


def _resolve_persistent_data_dir() -> Path:
    # This script is located in dev/python/dc-spect/ under the repository root.
...
def main(
    source_activity_bq: float = typer.Option(
        1e6, help="Source activity in Bq used with run timing intervals."
    ),
    chunk_duration_s: float = typer.Option(
        1.0, help="Duration of each run chunk in seconds."
    ),
    num_chunks: int = typer.Option(1, help="Number of run timing intervals (chunks)."),
    eventid_warn_threshold: float = typer.Option(
        1.5e9, help="Warn if expected events per chunk exceed this threshold."
    ),
    debug_geometry: bool = typer.Option(
        False,
        help="Dump the geometry tree and enable verbose Geant4 output before running.",
    ),
    num_threads: int = typer.Option(
        1, "-t", "--num-threads", help="Number of threads requested."
    ),
    xlsx_path: str | None = typer.Option(
        None, "-x", "--xlsx-path", help="Path to the geometry configuration xlsx file"
    ),
    ...
    output_dir: str = typer.Option(
        ".", "-o", "--output-dir", help="Directory to store the simulation output files"
    ),
    job_array_id: str | None = typer.Option(
        None, help="SLURM_ARRAY_JOB_ID used for naming output files."
    ),
    job_array_task_id: str | None = typer.Option(
        None, help="SLURM_ARRAY_TASK_ID used for naming output files."
    ),
):
    persistent_data_dir = _resolve_persistent_data_dir()
    resolved_job_array_id, resolved_job_array_task_id = _resolve_job_array_ids(
        job_array_id, job_array_task_id
    )
    ...
    args = SimpleNamespace(
        ...
        eventid_warn_threshold=eventid_warn_threshold,
        debug_geometry=debug_geometry,
        num_threads=num_threads,
        xlsx_path=xlsx_path,
        geometry_only=geometry_only,
        output_dir=output_dir,
        job_array_id=resolved_job_array_id,
        job_array_task_id=resolved_job_array_task_id,
    )

    if geometry_only:
        config = get_dc_spect_geometry_config(
            _resolve_xlsx_path(persistent_data_dir, xlsx_path),
            stl_dir=persistent_data_dir / "stl",
            n_pixels=(2, 2, 1),
        )
