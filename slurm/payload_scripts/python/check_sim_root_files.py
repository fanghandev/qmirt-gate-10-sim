...
def main() -> int:
    ...
    # Collect structured rows for output and keep summaries for optional TXT output
    ...
    try:
        if args.format == "csv":
            with out_path.open("w", newline="", encoding="utf-8") as fh:
                ...
                writer.writeheader()
                for r in rows:
                    # Ensure no non-string values cause issues
            ...
            with out_path.open("w", encoding="utf-8") as fh:
                ...
                fh.write(
                    f"OK: {ok_count}, Errors: {err_count}, Total TTrees: {total_trees}\n\n"
                )
                for s in summaries:
                    fh.write(f"File: {s.path}\n")
                    fh.write(f"  Size: {format_size(s.size_bytes)}\n")
                    if s.error:
                        fh.write("  Status: ERROR\n")
                        fh.write(f"  Message: {s.error}\n\n")
                        continue
                    fh.write("  Status: OK\n")
                    fh.write(f"  TTrees: {len(s.trees)}\n")
                    for t in s.trees:
